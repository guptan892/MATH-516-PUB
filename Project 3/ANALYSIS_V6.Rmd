...
#export
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_rank_pc_method.png", width = 1200, height = 1000, res = 150)
...
bootstrap_results_rank_pc <- replicate(bootstrap_sample_size, {
  ...
  fit_boot <- pc(suffStat = suff_boot, indepTest = gaussCItest,
                    labels = colnames(boot_data), alpha = 0.05, verbose = FALSE)
...
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/consensus_dag_rank_pc_bootstrap.png", width = 1200, height = 1000, res = 150)
par(mar = c(1, 1, 3, 1)) 
...
#export
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_test.png", width = 1200, height = 1000, res = 150)
...
#export
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_v2_test.png", width = 1200, height = 1000, res = 150)
...
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/correlation_matrix_default_test.png", width = 1000, height = 1000, res = 150)
corrplot(cor(model.matrix(~.-1, data=pc_data)), method="circle")
dev.off()
```
...
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_recentmonth_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# # 2. Export a high-quality version
# png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_cextend.png", width = 1600, height = 1200, res = 150)
# graphviz.plot(pc_fit_cextend, shape = "ellipse", 
#      main = "Full Causal DAG, PC Method")
# # 3. Close the device to save the file
# dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_maxsx_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_maxsx, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_cextend_maxsx_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_maxsx_cextend, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_limitedcov.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_limitedcov, shape = "ellipse", 
     main = "Causal DAG: PC Method, Limited Covariates")
# 3. Close the device to save the file
dev.off()
...
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/consensus_dag_pc_bootstrap_limitedcov.png", width = 1200, height = 1000, res = 150)
par(mar = c(1, 1, 3, 1)) 
plot(avg_graph_bootstrap_pc, 
     main = "Consensus DAG (Edges in >50% of Bootstraps): PC Method, Limited Covariates",
     vertex.size = 15,              # Smaller nodes
...
#export
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_fullcov.png", width = 1200, height = 1000, res = 150)
...
# Pc algorithm bootstrap nontransformed
```{r bootstrap pc method nontransformed full}
bootstrap_results_pc_full <- replicate(bootstrap_sample_size, {
  #bootstrap sample
  sample_df <- data[sample(nrow(data), replace = TRUE), ]
  ...
  # Run PC; alpha serves as a tuning parameter for the significance of independence tests
  pc_fit_boot <- pc(suffStat = sample_df_pc_data,
                      indepTest = mixCItest,
                      labels = colnames(sample_df_pc_data),
                      alpha = 0.05, 
                      verbose = FALSE,
                      u2pd = "relaxed", 
                      skel.method = "stable" ,  #order of test doesnt matter
  ...
  adj_matrix <- as(pc_fit_boot@graph, "matrix")
  adj_matrix_input <- pcalg2dagitty(adj_matrix, 
                        labels = colnames(sample_df_pc_data), 
                        type = "cpdag")
  ...
  # Update your covariate list for the models
  cov_pc_boot <- setdiff(y_indices_all_boot, "A") #exclude A
...
```{r bootstrap variance and bias for pc variables full}
orig_ace_pc_var_full <- c(ace_or_pc_var_full, ace_ipw_pc_var_full)

#extract ACE values
boot_aces_or_pc_full  <- unlist(lapply(bootstrap_results_pc_full, function(x) x$ace_or))
boot_aces_ipw_pc_full <- unlist(lapply(bootstrap_results_pc_full, function(x) x$ace_ipw))

#extract markov blanket
boot_blankets_pc_full <- lapply(bootstrap_results_pc_full, function(x) x$blanket)
boot_blankets_pc_a_full <- lapply(bootstrap_results_pc_full, function(x) x$blanket_a)

# convert to dataframe
boot_df_pc_var_full <- data.frame(
  Regression = boot_aces_or_pc_full,
  IPW = boot_aces_ipw_pc_full
)
colnames(boot_df_pc_var_full) <- c("Regression", "IPW")

# Calculate Metrics
metrics_pc_var_full <- data.frame(
  Method = c("Regression", "IPW"),
  Original_ACE = orig_ace_pc_var_full, # Use values from the full dataset
  Bootstrap_ACE = c(mean(boot_df_pc_var_full$Regression), mean(boot_df_pc_var_full$IPW)),
  Standard_Error = c(sd(boot_df_pc_var_full$Regression), sd(boot_df_pc_var_full$IPW))
)

# Calculate Bias
metrics_pc_var_full$Bootstrap_Bias <- metrics_pc_var_full$Bootstrap_ACE - metrics_pc_var_full$Original_ACE
print(metrics_pc_var_full)
```

```{r markov blankets full}
boot_blankets_pc_unlisted_full <- unlist(boot_blankets_pc_full)

boot_mb_stability_pc_full <- table(boot_blankets_pc_unlisted_full) / bootstrap_sample_size
print("Selection Frequency for Y's Markov Blanket:")
print(sort(boot_mb_stability_pc_full, decreasing = TRUE))
```

```{r markov blankets a full}
boot_blankets_pc_a_unlisted_full <- unlist(boot_blankets_pc_a_full)

boot_mb_stability_pc_a_full <- table(boot_blankets_pc_a_unlisted_full) / bootstrap_sample_size
print("Selection Frequency for A's Markov Blanket:")
print(sort(boot_mb_stability_pc_a_full, decreasing = TRUE))
```

```{r DAG avg full, fig.width=10, fig.height=8, out.width="100%"}
#get adjacent matrices
all_adj_bootstrap_pc_full <- lapply(bootstrap_results_pc_full, function(x) x$adj_matrix)
#sum them up to get edge frequencies - prob an edge exists between 2 variables
consensus_matrix_bootstrap_pc_full <- Reduce("+", all_adj_bootstrap_pc_full) / bootstrap_sample_size #reduce sums all matrices. if arrow from pay_0 to y is in every bootstrap, then it will have bootstrap_sample_size values in that matrix cell. if only half the time, then half the size.

# Create a graph where an edge exists if it appeared in > 50% of samples
average_adj_bootstrap_pc_full <- (consensus_matrix_bootstrap_pc_full > 0.5) * 1

# Convert back to a graph object for plotting
avg_graph_bootstrap_pc_full <- graph_from_adjacency_matrix(average_adj_bootstrap_pc_full, mode = "directed")
plot(avg_graph_bootstrap_pc_full, main = "Consensus DAG (Edges in >50% of Bootstraps)")

png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/consensus_dag_pc_bootstrap_full.png", width = 1200, height = 1000, res = 150)
par(mar = c(1, 1, 3, 1)) 
plot(avg_graph_bootstrap_pc_full, 
     main = "Consensus DAG (Edges in >50% of Bootstraps): PC Method, All Covariates",
     vertex.size = 15,              # Smaller nodes
     vertex.label.cex = 0.6,        # Smaller labels
     vertex.label.dist = 1.2,       # Offset labels from node centers
     edge.arrow.size = 0.3,         # Smaller arrows
...
Use MMPC from bnlearn. The max-min parents and children algorithm. This searches the neighborhood of Y (parents and children) by testing for conditional independence at the alpha=0.05 level. Variable is only kept if relationship with Y can't be explained away by other variables.
...
local_y_test <- learn.mb(local_data_test_mbonly, node = "Y", method = "iamb", alpha = 0.05, test="cor")
...
local_a_test <- learn.mb(local_data_test_mbonly, node = "A", method = "iamb", alpha = 0.05, test="cor")
...
```{r}
# learn.mb(local_data, node = "A", method = "a", alpha = 0.05)
...
# Run nbr to find the markov blanket of Y only
local_y <- learn.mb(local_data_test_mbonly, node = "Y", method = "inter.iamb", alpha = 0.05, test = "cor")
...
#now do the same for A to get covariates for propensity score model
local_a <- learn.mb(local_data_test_mbonly, node = "A", method = "inter.iamb", alpha = 0.05, test = "cor")
print("Variables in the Local Neighborhood (Parents/Children) of A:")
print(local_a)
...
bootstrap_results_local <- replicate(bootstrap_sample_size, {
  ...
  local_y_boot <- tryCatch({
    learn.mb(sample_df_ranked, node = "Y", method = "inter.iamb", test = "cor", alpha = 0.05)
  }, error = function(e) character(0))
  
  cov_local_boot <- setdiff(local_y_boot, "A")
  
  local_a_boot <- tryCatch({
    learn.mb(sample_df_ranked, node = "A", method = "inter.iamb", test = "cor", alpha = 0.05)
  }, error = function(e) character(0))
...
# Run nbr to find the markov blanket of Y only
local_y_blacklist <- learn.mb(local_data, node = "Y", method = "inter.iamb", blacklist = master_blacklist_matrix_excl_y, alpha = 0.05)
...
#now do the same for A to get covariates for propensity score model
local_a_blacklist <- learn.mb(local_data, node = "A", method = "inter.iamb", blacklist = master_blacklist_matrix_excl_a, alpha = 0.05)
print("Variables in the Local Neighborhood (Parents/Children) of A:")
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_blacklist_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_blacklist, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_cextend_blacklist_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_cextend_blacklist, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_maxsx_blacklist_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_maxsx_blacklist, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
# 2. Export a high-quality version
png("OUTPUT_IMAGES_FROM_ANALYSIS_V6/causal_dag_pc_method_bnlearn_cextend_maxsx_blacklist_test.png", width = 1600, height = 1200, res = 150)
graphviz.plot(pc_fit_bnlearn_maxsx_cextend_blacklist, shape = "ellipse", 
     main = "TEST Full Causal DAG, PC Method")
# 3. Close the device to save the file
dev.off()
...
ggsave(filename = "OUTPUT_IMAGES_FROM_ANALYSIS_V6/ACE_Comparison_Table.png", 
       plot = tab_plot, 
...
# gtsave(final_table_img, filename = "OUTPUT_IMAGES_FROM_ANALYSIS_V6/ACE_Comparison_Table.png")
...
ggsave("OUTPUT_IMAGES_FROM_ANALYSIS_V6/ace_standard_errors.png", plot = ace_se_graph, width = 8, height = 6, dpi = 300)
print(ace_se_graph)
...
ggsave("OUTPUT_IMAGES_FROM_ANALYSIS_V6/ace_confidence_intervals.png", plot = ace_ci_graph, width = 8, height = 6, dpi = 300)
print(ace_ci_graph)
```
